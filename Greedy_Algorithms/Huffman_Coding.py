...
Преимущества:
* Эффективная компрессия для данных с неравномерной частотой символов.
* Гарантированная без потерь (lossless) компрессия.
* Широко используется в различных форматах сжатия, таких как JPEG и MP3.

Недостатки:
* Требует предварительного анализа частоты символов, что добавляет накладные расходы.
* Не оптимальный для очень коротких сообщений.
* Неэффективен для данных с равномерным распределением символов.
"""